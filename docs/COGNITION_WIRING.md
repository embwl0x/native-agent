...
| **Memory** (`MemoryV2` + KG) | **On `.active` turns: memory arrives in the CONTEXT PACKET, not `TurnContext.recalled`** — the legacy top-k lane is skipped outright (`ChatOrchestration+TurnEngine.swift:806-810`, outcome `.contextFlow` **:810**). `recalled` is non-empty only on `off`/`shadow`/fallback turns **[confirmed** recall `MemoryV2+Storage.swift:802`**]** | `commit_memory` writes; chat turns **[partial]** | two separate budgets — **selection-side** 12 dynamic atoms, ≤2 per source, `.memory` kind quota 8; **packet-side** 6,000 chars. Both tables in full below ([Context budgets](#context-budgets-selection-side-and-packet-side)); **NOT persona-scoped** — `memoryRecallPersonaFilter` resolves every slot to `nil` (`+TurnEngine.swift:44`); per-record disclosure is the real boundary (`MemoryRecordDisclosure.swift:47`) |
...
Two different ceilings, in two different files. Quoting one as "the" context
budget is how the old "6,000-char packet budget, ≤2 atoms/source" line read as
the whole story.

**Selection side** — how many atoms may be chosen.
`ContextSelectionConfiguration.init`,
`Modules/NativeAgentCore/Sources/Context/ContextSelectionContracts.swift:1013-1027`:

| Knob | Default |
|---|---|
| `maximumCandidates` | 256 |
| `maximumDynamicAtoms` | **12** |
| `maximumPointers` | 8 |
| `maximumAtomsPerSource` | **2** |
| `maximumAtomsPerKind` | 4 |
| `maximumAtomsPerKindOverrides` | `.memory: 8`, `.relationship: 4` |
| `reservedRoleSlotsPerKind` | `.memory`: 2 slots reserved for `.procedure` |
| `maximumCorrectionAtomsPerTurn` | 3 (ambient corrections only) |
| `minimumRelevance` | 0.05 |
| `memorySemanticFloor` | 0.30 cosine |
| `shortMessageMemoryRowCap` | 6, on messages of ≤4 content tokens |

So the per-source cap really is 2, but the per-*kind* cap is 4 with `.memory`
overridden to **8** — not 2 — and the whole dynamic set is capped at 12.

**Packet side** — how many characters the selected atoms may spend.

| Knob | Default | Source |
|---|---|---|
| `ContextNeed.characterBudget` | 6,000 | `ContextSelectionContracts.swift:154` |
| `ContextTurnRequest.characterBudget` | 6,000 | `ContextTurnRuntime.swift:236` |
| `ContextBudgetPolicy.floorPacketChars` | 6,000 | `ChatOrchestration/ContextBudgetPolicy.swift:208` |
| `ContextBudgetPolicy.maximumPacketCharacters` | **32,000** | `ChatOrchestration/ContextBudgetPolicy.swift:83` |
| `ContextExpansionConfiguration.maximumCharacters` | 12,000 | `Context/ContextExpansion.swift:6` |

6,000 is the **floor**, not a fixed ceiling: the live turn passes
`packetBudget.packetChars` (`ChatOrchestration+TurnEngine.swift:463`), which
scales the floor with the model's window up to 32,000
(`ContextBudgetPolicy.swift:336`). On-demand atom expansion has its own separate
12,000-char limit.
...
| **Autonomy tier promotion** | `BackgroundLoops/AutonomyPromotionLoop.swift:36` (`loopId "autonomy_promotion_proposals"` **:37**, 3,600 s default **:53** — production overrides it to `24 * 60 * 60`, `BackgroundLoopsAssembly+Autonomy.swift:44`, tick **:141**) | daily (approval/policy mutations and the persisted proposal cooldown drive normal work) | approval cards only | **never self-flips** (**:8-9**); `promotableTiers = ["confirm","supervised"]` **:75**; card re-verified before apply **:171-176** |
