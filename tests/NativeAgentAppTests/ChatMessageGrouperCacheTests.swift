...
// append, wholesale reload), the cached `groups(for:sessionId:)` must equal
// `_compute(for:)` — the uncached ground truth.
...
struct ChatMessageGrouperCacheTests {
    ...
    @Test func cachedGroupingEqualsRecomputeForEveryLiveMutationShape() {
        ...
        func check(_ label: String) {
            let cached = MessageGrouper.groups(for: messages, sessionId: sessionId)
        ...
        check("equal-length last-row replacement")
    ...
    @Test func interleavedSessionsNeverShareACacheSlot() {
        ...
        for step in 0..<6 {
            ...
            let groupsA = MessageGrouper.groups(for: messagesA, sessionId: a)
            let groupsB = MessageGrouper.groups(for: messagesB, sessionId: b)
    ...
    @Test func fifoEvictionDegradesToRecomputeNotToAWrongGrouping() {
        ...
        for (index, sid) in sessionIDs.enumerated() {
            ...
            _ = MessageGrouper.groups(for: transcripts[sid]!, sessionId: sid)
        }
        ...
        for sid in sessionIDs {
            ...
            #expect(
                grouperProjection(MessageGrouper.groups(for: messages, sessionId: sid))
