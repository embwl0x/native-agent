...
@Suite("uniform file locking — SelfImprovement")
struct SelfImprovementUniformFileLockTests {
    /// `withTrainingFileLock` used to downcast to SwiftNativePersistenceCore and
    /// run the proposal read-modify-write BARE otherwise. Six actors reject the
    /// SAME proposal file concurrently, so they all contend for one lock.
    @Test func trainingFileLock_serializesForNonSwiftNativeConformer() async throws {
        let root = lockProbeTempRoot("selfimprove")
        defer { try? FileManager.default.removeItem(at: root) }
        let probe = LockProbePersistence()
        let proposalsDir = root.appendingPathComponent("training_journal/proposals", isDirectory: true)
        try FileManager.default.createDirectory(at: proposalsDir, withIntermediateDirectories: true)
        let proposalPath = proposalsDir.appendingPathComponent("0001.json")
        try await probe.writeJSON(.object([
            "proposal_id": .string("p-1"),
        ...
        await withTaskGroup(of: Void.self) { group in
            for i in 0..<6 {
                group.addTask {
                    let client = SwiftNativeSelfImprovement(persistence: probe, dataRoot: root)
                    _ = try? await client.rejectTrainingProposalLocal(
                        proposalId: "p-1", reason: "reason-\(i)"
                    )
                }
            }
        }
        ...
        #expect(
            probe.peakConcurrency(proposalPath) == 1,
            "the proposal read-modify-write must be mutually exclusive for every conformer"
        )
        let raw = await probe.readJSON(proposalPath, defaultValue: .object([:]))
        guard case .object(let obj) = raw else {
            Issue.record("proposal file was not an object")
            return
        }
        #expect(obj["status"] == .string("rejected"))
    }
}
