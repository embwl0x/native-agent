...
@Suite("uniform file locking — MCPDispatcher")
struct MCPDispatcherUniformFileLockTests {
    /// grantConsent read-modify-writes the MCP consent ledger — an
    /// AUTHORIZATION store, so a lost update is a lost grant/revoke.
    @Test func grantConsent_serializesForNonSwiftNativeConformer() async throws {
        ...
        await withTaskGroup(of: Void.self) { group in
            for i in 0..<6 {
                group.addTask {
                    let peer = SwiftNativeMCPDispatcher(root: root, persistence: probe)
                    _ = try? await peer.grantConsent(MCPConsentGrant(
                        serverId: "server-\(i)", toolName: "tool-\(i)", risk: "low"
                    ))
                }
            }
        }
        ...
        #expect(
            probe.peakConcurrency(ledgerPath) == 1,
            "the consent ledger read-modify-write must be mutually exclusive for every conformer"
        )
        let raw = await probe.readJSON(ledgerPath, defaultValue: .array([]))
        guard case .array(let rows) = raw else {
            Issue.record("consent ledger was not an array")
            return
        }
        #expect(rows.count == 6, "an unlocked R-M-W silently drops granted consents")
    }
}
