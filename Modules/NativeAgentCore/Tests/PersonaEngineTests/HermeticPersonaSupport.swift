...
// into the user's LIVE feed on every suite run, and makes persona compilation
// READ the user's real profile.json (machine-dependent test outcomes).
//
...
// and already has its own data root in scope (CompilerTests already does this
// by hand — see its `makeCompiler(root:dataRoot:)`).
//
// Helpers do not cross target boundaries; this file is the PersonaEngineTests
...
func hermeticPersonaDataRoot() -> URL {
    let root = FileManager.default.temporaryDirectory
        .appendingPathComponent("PersonaEngineTests-personaData-\(UUID().uuidString)", isDirectory: true)
