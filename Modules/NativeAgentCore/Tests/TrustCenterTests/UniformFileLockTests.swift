...
@Suite("uniform file locking — TrustCenter")
struct TrustCenterUniformFileLockTests {
    /// upsertCatalogSource read-modify-writes catalog/sources/sources.json under
    /// withSourcesLock. Six actors upsert six distinct sources concurrently.
    @Test func upsertCatalogSource_serializesForNonSwiftNativeConformer() async throws {
        let root = lockProbeTempRoot("trustcenter")
        defer { try? FileManager.default.removeItem(at: root) }
        let probe = LockProbePersistence()
        let sourcesPath = root.appendingPathComponent("catalog/sources/sources.json")

        await withTaskGroup(of: Void.self) { group in
            for i in 0..<6 {
                group.addTask {
                    let writes = SwiftNativeCatalogWrites(dataRoot: root, persistence: probe)
                    _ = try? await writes.upsertCatalogSource([
                        "id": .string("source-\(i)"),
                        "name": .string("Source \(i)"),
                        "kind": .string("local"),
                    ])
                }
            }
        ...
        #expect(
            probe.peakConcurrency(sourcesPath) == 1,
            "sources.json read-modify-write must be mutually exclusive for every conformer"
        )
        let raw = await probe.readJSON(sourcesPath, defaultValue: .array([]))
        guard case .array(let rows) = raw else {
            Issue.record("sources.json was not an array")
            return
        }
        ...
        for i in 0..<6 {
            #expect(ids.contains("source-\(i)"), "an unlocked R-M-W loses concurrent catalog upserts")
        }
    }
}
