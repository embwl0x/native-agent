...
@Suite("uniform file locking — ToolRegistry")
struct ToolRegistryUniformFileLockTests {
    /// promote() read-modify-writes tools/registry.json. Six registry actors
    /// promote six distinct records concurrently over a NON-SwiftNative conformer.
    @Test func promote_serializesForNonSwiftNativeConformer() async throws {
        let root = lockProbeTempRoot("toolregistry")
        defer { try? FileManager.default.removeItem(at: root) }
        let probe = LockProbePersistence()
        let registryPath = root.appendingPathComponent("tools/registry.json")

        let seeded: [JSONValue] = (0..<6).map { i in
            .object([
                "id": .string("tool-\(i)"),
                "name": .string("Tool \(i)"),
        ...
        try await probe.writeJSON(.array(seeded), to: registryPath)

        await withTaskGroup(of: Void.self) { group in
            for i in 0..<6 {
                group.addTask {
                    let registry = SwiftNativeToolRegistry(root: root, persistence: probe)
                    _ = try? await registry.promote(id: "tool-\(i)")
                }
            }
        }

        #expect(
            probe.peakConcurrency(registryPath) == 1,
            "registry.json read-modify-write must be mutually exclusive for every conformer"
        )
        let raw = await probe.readJSON(registryPath, defaultValue: .array([]))
        guard case .array(let rows) = raw else {
            Issue.record("registry.json was not an array")
            return
        }
        let active = rows.filter {
            guard case .object(let o) = $0, case .string("active")? = o["status"] else { return false }
            return true
        }
        #expect(active.count == 6, "an unlocked R-M-W loses concurrent promotes")
    }
}
