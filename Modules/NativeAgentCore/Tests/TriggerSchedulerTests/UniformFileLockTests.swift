...
@Suite("uniform file locking — TriggerScheduler")
struct TriggerSchedulerUniformFileLockTests {
    /// SchedulerJobs.createJob read-modify-writes scheduler/jobs.json. Six
    /// independent scheduler actors (the multi-instance shape the app actually
    /// runs) submit concurrently through a NON-SwiftNative conformer.
    @Test func createJob_serializesForNonSwiftNativeConformer() async throws {
        let root = lockProbeTempRoot("scheduler")
        ...
        await withTaskGroup(of: Void.self) { group in
            for i in 0..<6 {
                group.addTask {
                    let scheduler = SwiftNativeTriggerScheduler(root: root, persistence: probe)
                    _ = try? await scheduler.createJob(body: .object([
                        "name": .string("job-\(i)"),
                        "kind": .string("notify"),
                        "intervalSeconds": .int(3600),
                        "payload": .object(["message": .string("ping \(i)")]),
                    ]))
                }
            }
        }
        ...
        #expect(
            probe.peakConcurrency(jobsPath) == 1,
            "jobs.json read-modify-write must be mutually exclusive for every conformer"
        )
        let raw = await probe.readJSON(jobsPath, defaultValue: .array([]))
        guard case .array(let jobs) = raw else {
            Issue.record("jobs.json was not an array")
            return
        }
        #expect(jobs.count == 6, "an unlocked R-M-W loses concurrent job creates")
    }
}
