    ...
    func appendOptionalSchemas(
                ...
                requestedSchema(
                    ...
                    parametersJSON: params(
                        properties: [
                            ...
                            ("cwd", strSchema("Optional existing working directory. Relative paths resolve in the canonical NativeAgent workspace/source checkout. Active Full Mac YOLO may select an ordinary absolute external project; sensitive authority and protected system paths remain denied.")),
                            ("three_way", nullableRecallField(boolSchema("Optional. Default true (passes --3way to git apply). Set false for strict apply that fails fast on context mismatch."))),
                        ],
                        required: ["patch"]
                    )
                ),
        ...
        if includeFullMacAccessibilityInjectionTools {
            ...
            schemas.append(contentsOf: [
                requestedSchema(
                    name: "act",
                    description: "Do a whole flow in ONE call with steps:[…] — names, menu paths ('Format > Make Plain Text'), chords, {verb,target,text}; each verified on a fresh read, stops at first failure. One receipt line per step; a step ending in ? ('OK?') is skipped if absent. Menus and keys need app plus front:true. E.g. save as plain text on the Desktop: app:'TextEdit' front:true steps:['Format > Make Plain Text','OK?','cmd+s',{verb:'type',target:'Save As',text:'hello.txt'},'cmd+d','return'] (cmd+d = Desktop in a save dialog). Single act: verb + target, by name or visible ordinal, against a fresh fused screen. Semantic: click/open/type/select/toggle/scroll/dismiss. Physical: hover, move, drag (give `to`), hold, key (target a key/chord sequence like `cmd+s` or `1 2 3`; `hold` can target `key w`). Unlabeled pixel objects appear as numbered visual regions and take physical actions only. `repeat` runs a short burst, re-resolving the target before every attempt so moving targets are followed; ambiguity, drift, a vanished target or the time limit stops it. `holding` keeps keys/modifiers down around a physical move, drag, click, scroll or key (e.g. `w d` while dragging a world view). Accessibility targets use the app's own action; pixel-only ones use the bounded physical hand. Pass app to act in that app's window in the background without taking the front; if it truly needs the front it answers needs_front, and front:true brings it forward briefly, then restores the previous front app. Needs active Full Mac Accessibility app control; there is no per-call approval.",
                    parametersJSON: params(
                        properties: [
                            ("verb", enumStringSchema(["click", "open", "type", "select", "toggle", "scroll", "dismiss", "hover", "move", "drag", "hold", "key", "press"], "What to do. press = key for a key/chord (cmd+s, return), click for a named control.")),
                            ...
                            ("steps", obj([
                                ("type", .string("array")),
                                ("maxItems", .int(12)),
                                ("description", .string("Optional: up to 12 acts in ONE call, in order — [\"AC\",\"9\",\"×\",\"8\",\"=\"] (a string presses/clicks that name, a menu path like 'File > Save', or a key/chord) or {verb, target, text}. Each step reads the window fresh and is verified; it stops at the first failure and says which step. With steps, verb/target are ignored; app/front apply to all.")),
                                ("items", obj([("anyOf", .array([
                                    obj([("type", .string("string"))]),
                                    obj([
                                        ("type", .string("object")),
                                        ("properties", obj([
                                            ("verb", strSchema("click, open, type, select, toggle, scroll, dismiss, key or press.")),
                                            ("target", strSchema("The name, as the screen shows it.")),
                                            ("text", strSchema("For type: the text.")),
                                        ])),
                                        ("required", .array([.string("verb"), .string("target")])),
                                    ]),
                                ]))])),
                            ])),
                            ("app", strSchema("Optional: act in this running app's window (\"TextEdit\", \"Mail\") without bringing it forward. Works for accessibility clicks, typing, select, toggle and open; menus, hover, drag, keys and pixel targets answer needs_front instead.")),
                            ("front", boolSchema("With app: bring that app forward for this act (menus, keys), then put the previous front app back. A name it lacks raises nothing. The result says how long it was in front.")),
