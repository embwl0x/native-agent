...
enum CodexImageGenerationHelp {
    ...
    Example generation: prompt='A polished brass telescope on a teal base with a plaque reading the agent, isolated transparent cutout', background=transparent. Example edit: prompt='Change only the base to violet; preserve the telescope and the agent plaque', action=edit, referenced_image_paths=['/absolute/path/from/previous/result.png']. Use composition, lighting, exact quoted text, preservation goals and desired detail in the prompt. Inspect pixels after each edit.
