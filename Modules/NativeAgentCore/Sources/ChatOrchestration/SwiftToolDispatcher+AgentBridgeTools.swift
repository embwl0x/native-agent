...
extension SwiftToolDispatcher {
            ...
            appendResult = try await persistence.withFileLock(inboxURL) {
                ...
                if case .object(let object)? = existing {
                    guard object["text"] == .string(text),
                          ...
                          (object["requireExistingConversation"] == .bool(true)) == requireExistingConversation,
                          object["workingDirectory"] == inboxEntry["workingDirectory"],
                          object["deskHandle"] == inboxEntry["deskHandle"],
                          object["pairReviewer"] == inboxEntry["pairReviewer"] else {
                        return ("conflict", false, timestamp)
                    }
                    if let session = Self.stringField("sessionId", in: .object(object)), !session.isEmpty {
                        guard object["sessionId"] == inboxEntry["sessionId"],
                              object["priority"] == inboxEntry["priority"],
                              object["timeoutSeconds"] == inboxEntry["timeoutSeconds"] else {
                            return ("conflict", false, timestamp)
                        }
                    }
                    return ("duplicate", Self.builderInboxAllowsExplicitWakeRetry(object),
            ...
            appendResult = try await persistence.withFileLock(inboxURL) {
                ...
                if case .object(let object)? = existing {
                    guard object["text"] == .string(text),
                          ...
                          (object["requireExistingConversation"] == .bool(true)) == requireExistingConversation,
                          object["workingDirectory"] == inboxEntry["workingDirectory"],
                          object["deskHandle"] == inboxEntry["deskHandle"] else {
                    ...
                    return ("duplicate", Self.builderInboxAllowsExplicitWakeRetry(object),
                            Self.stringField("createdAt", in: .object(object)) ?? queuedAt)
                }
                try await appendJSONLCapped(
                    .object(inboxEntry),
                    to: inboxURL,
    ...
    ///
    /// This closes the unresolved HIGH review note below: nothing held two
    /// concurrent invokes apart (a Mac-chat turn and a Telegram turn run in the
    /// SAME process), and the live evidence was a `.stale-<ts>` rename-aside
    /// produced by one invoke's reset landing on another's pointer.
    ///
    ...
    ///
    /// LOCK_NB with a bounded spin, and an unlocked fall-through if the lock
    /// cannot be taken at all: both critical sections are one small read and
    /// one small write, and wedging a chat turn on a lock sidecar would be a
    /// worse failure than the race this removes. The fall-through is exactly
    /// the pre-lock behaviour — never worse than today.
    ...
    static func runInvokeClaude(
        ...
        // RESOLVED (was gpt-5.5 review HIGH, "no lock around the session yet"):
        // the read/check/reset below and the write/reset in the termination
        // handler are each serialized by `withSessionPointerLock`, and each
        // re-reads the live pointer under that lock. Two invokes in one process
        // (Mac chat + Telegram) can no longer interleave here. The conservative
        // rename-aside reset semantics are unchanged — the lock only adds who
        // may run them, and the re-read only adds that a reset must still be
        // aimed at the pointer this run actually resumed.
        let pointerProvenance = dataRoot.standardizedFileURL.path
        let pointerDecision: (id: String, args: [String], isNew: Bool, cwd: String)
        ...
        let result: JSONValue = await withCheckedContinuation { (cont: CheckedContinuation<JSONValue, Never>) in
            ...
            process.terminationHandler = { proc in
                ...
                Self.withSessionPointerLock(sessionFile) {
                    if isNewSession {
                        if exitCode == 0 {
