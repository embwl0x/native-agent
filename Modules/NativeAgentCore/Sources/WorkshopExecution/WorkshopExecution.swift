...
//   mission enqueue but landed empty-plan missions because it skipped
...
//     adapters. One checked routing snapshot admits the complete 'missions'
...
//     drains queued missions (claim under flock, started/step_completed/
...
//     `plan: []` mission (the wave-12 regression).
//   - PARITY GAP (wave 23 known limitation). Python's `run_codex` resolves
//     the ACTIVE PROVIDER for the missions surface via daemon
...
//   - CANCELLATION (wave 23). `runCodex` and `_planMissionWithReason`
...
//     silently land a stub mission.
//   - title/objective truncation matches Python: title[:160],
//     objective[:2000].
//   - mission.id is a v4 UUID lowercased to match Python's str(uuid.uuid4()).
...
//   - asdict(mission) shape parity: every field on the Python @dataclass
//     Mission appears in the JSON with the same key name (all 15 fields).
