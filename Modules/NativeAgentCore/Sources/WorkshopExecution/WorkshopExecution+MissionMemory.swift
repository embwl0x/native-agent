...
// disclose mission memories. Nothing ever wrote one. Every non-test hit for that
...
// mission she ran was work she could not remember doing.
//
// This file is the plug. On every TERMINAL Workshop execution the executor
// records one prose memory of what the mission was, what came of it, and whether
...
// A deliberate consequence: re-running the same mission to the same outcome
...
/// One mission memory as retention sees it.
public struct WorkshopMissionMemoryHandle: Sendable, Equatable {
    ...
    /// mission that produced it, and what startup reconciliation matches on.
...
/// Tests substitute a fake; production uses ``SwiftNativeWorkshopMissionMemoryWriter``.
public protocol WorkshopMissionMemoryWriting: Sendable {
    /// Returns the stored record id.
    func writeMissionMemory(content: String, source: String, metadata: JSONValue) async throws -> String
    /// Every mission memory currently ACTIVE, for the retention sweep.
    func missionMemories(sourcePrefix: String) async throws -> [WorkshopMissionMemoryHandle]
    /// Retire one mission memory out of the lane WITHOUT minting a content
    /// tombstone — see ``WorkshopMissionMemory/retentionCap`` for why deletion
...
public struct SwiftNativeWorkshopMissionMemoryWriter: WorkshopMissionMemoryWriting {
    ...
    public func writeMissionMemory(
        content: String,
        source: String,
        metadata: JSONValue
    ) async throws -> String {
    ...
    public func missionMemories(sourcePrefix: String) async throws -> [WorkshopMissionMemoryHandle] {
        try await memory.memoryHandles(
            sourcePrefix: sourcePrefix,
            limit: WorkshopMissionMemory.retentionCap + WorkshopMissionMemory.retentionSweepWindow
        )
        .map { WorkshopMissionMemoryHandle(id: $0.id, createdAt: $0.createdAt, source: $0.source) }
    }
    ...
    /// lane: the next mission that produces the same prose is refused as
    ...
    public func retireMissionMemory(id: String) async throws {
...
/// Pure, testable decisions about what a finished mission is worth remembering.
public enum WorkshopMissionMemory {
    ...
    /// taxonomy with a 60-day recall half-life — mission history should fade in
    ...
    /// a mission-per-completion write would slowly crowd User's facts out of a
    ...
    /// ``SwiftNativeWorkshopMissionMemoryWriter/retireMissionMemory(id:)``.
    ...
    /// Clip for free text lifted out of a mission (result payloads, error
    ...
    ///
    /// The temptation is to filter "trivial" missions. Resisted: a mission is
    ...
    /// nothing to remember is a mission cancelled before a single step ran —
    /// no work happened, so there is no outcome to recall. Everything else is
    /// remembered, and every drop is logged with its reason.
    ///
    /// Note what is NOT filtered: failures. A mission that failed on step one
    ...
    /// What she should actually remember: what the mission was, what came of it,
    ...
    public static func narrative(_ record: WorkshopExecutionRecord, reason: String?) -> String {
        ...
        switch status {
        case "completed":
            sentences.append("Workshop mission \"\(name)\" completed.")
        case "failed":
            sentences.append("Workshop mission \"\(name)\" failed.")
        case "cancelled":
            sentences.append("Workshop mission \"\(name)\" was cancelled before it finished.")
        default:
            sentences.append("Workshop mission \"\(name)\" ended as \(status).")
        }
        ...
        //    is the difference between "the steps ran" and "the mission worked".
        if let verification = record.verification {
            switch verification.status {
            case .satisfied:
                sentences.append("The outcome was verified against the mission's own success criterion.")
            case .failed:
        ...
        // 6. Provenance that changes how much weight it deserves: a mission she
    ...
    /// Telegram, iOS, the agent bridge and missions, and never the
    ...
    public static func metadata(_ record: WorkshopExecutionRecord, reason: String?) -> JSONValue {
        ...
        var object: [String: JSONValue] = [
            ...
            "tags": .array([.string("workshop"), .string("mission"), .string("mission:\(record.status)")]),
    ...
    static func missionName(_ record: WorkshopExecutionRecord) -> String {
...
public enum WorkshopMissionMemoryLogLevel: String, Sendable, Equatable {
    case info
    case error
}

public typealias WorkshopMissionMemoryLog =
    @Sendable (WorkshopMissionMemoryLogLevel, String) -> Void

// MARK: - Recorder

/// Applies ``WorkshopMissionMemory`` to a finished execution: decide, write,
/// then bound the lane. Never throws — a memory failure must not change what a
/// mission's terminal state is — but it is never silent either: every drop,
/// every failure and every retention deletion is logged.
public struct WorkshopMissionMemoryRecorder: Sendable {
    private let writer: any WorkshopMissionMemoryWriting
    private let log: WorkshopMissionMemoryLog
    ...
    ///     takes the default.
    public init(
        writer: any WorkshopMissionMemoryWriting,
        retentionCap: Int = WorkshopMissionMemory.retentionCap,
        log: @escaping WorkshopMissionMemoryLog = { _, message in
            NSLog("%@", message)
        }
    ) {
        self.writer = writer
    ...
    public var logSink: WorkshopMissionMemoryLog { log }
    ...
    public func record(_ record: WorkshopExecutionRecord, reason: String?) async -> String? {
        switch WorkshopMissionMemory.decide(record) {
        case .drop(let why):
            log(.info, "[workshop-memory] DROPPED mission \(record.id) (status \(record.status)): \(why)")
            return nil
        case .remember:
            break
        }

        let content = WorkshopMissionMemory.narrative(record, reason: reason)
        let id: String
        do {
            id = try await writer.writeMissionMemory(
                content: content,
                source: WorkshopMissionMemory.source(for: record),
                metadata: WorkshopMissionMemory.metadata(record, reason: reason)
            )
        } catch {
            // Loud, then closed. The mission's terminal state is already durable;
            ...
            log(.error, "[workshop-memory] ERROR writing mission \(record.id) memory: \(error)")
            return nil
        }
        log(.info, "[workshop-memory] remembered mission \(record.id) (status \(record.status)) as memory \(id)")
        await pruneToCap()
        return id
    }

    /// Every mission memory this lane can currently see, by `source`. The
    /// startup crash-reconciliation read — see
    /// ``WorkshopExecutorLoop/reconcileMissedMissionMemories(within:maxRecords:)``.
    public func recordedSources() async -> Set<String> {
        do {
            let handles = try await writer.missionMemories(
                sourcePrefix: WorkshopMissionMemory.sourcePrefix
            )
            return Set(handles.compactMap(\.source))
        } catch {
            log(.error, "[workshop-memory] ERROR listing mission memories for reconciliation: \(error)")
            // An EMPTY set would read as "nothing is remembered" and re-write
            // every recent mission. Nil-shaped failure instead: the caller gets
            // no sources and reconciliation skips, loudly.
            return []
        }
    }

    /// Bounded-lane sweep — see ``WorkshopMissionMemory/retentionCap``.
    ...
    private func pruneToCap() async {
        var pass = 0
        while pass < WorkshopMissionMemory.retentionMaxPasses {
            pass += 1
            let handles: [WorkshopMissionMemoryHandle]
            do {
                handles = try await writer.missionMemories(sourcePrefix: WorkshopMissionMemory.sourcePrefix)
            } catch {
                log(.error, "[workshop-memory] ERROR listing mission memories for retention: \(error)")
                return
            }
            ...
            log(
                .info,
                "[workshop-memory] retention: \(handles.count) mission memories exceeds cap "
                + "\(retentionCap); archiving \(evicted.count) oldest (pass \(pass))"
            )
            var archived = 0
            for handle in evicted {
                do {
                    try await writer.retireMissionMemory(id: handle.id)
                    archived += 1
                    log(
                        .info,
                        "[workshop-memory] retention: archived mission memory \(handle.id) (created \(handle.createdAt))"
                    )
                } catch {
                    log(.error, "[workshop-memory] ERROR archiving mission memory \(handle.id): \(error)")
                }
            }
        ...
        log(
            .error,
            "[workshop-memory] retention: gave up after \(WorkshopMissionMemory.retentionMaxPasses) passes; "
...
/// Serialized, fire-and-forget mission-memory writes.
///
...
/// on to the next queued mission even though the terminal CAS and the Desk
/// receipt had already landed. Her recollection of a mission must never be able
/// to hold up the next one.
///
/// The write is not LOST by being asynchronous: every enqueue is chained onto
/// the previous one (so mission order is preserved and two writes never race
...
public actor WorkshopMissionMemoryQueue {
    ...
    private let recorder: WorkshopMissionMemoryRecorder
    private let log: WorkshopMissionMemoryLog
    ...
    public init(
        recorder: WorkshopMissionMemoryRecorder,
        maxPending: Int = WorkshopMissionMemoryQueue.defaultMaxPending,
        log: WorkshopMissionMemoryLog? = nil
    ) {
    ...
    /// for the store.
    ///
    /// At the bound the write is DROPPED, and dropping is LOUD: the mission's
    ...
    /// writer, so the rows already queued are older missions that would
    ...
    public func enqueue(_ record: WorkshopExecutionRecord, reason: String?) {
        guard pending < maxPending else {
            ...
            log(
                ...
                + "mission memory for \(record.id) (status \(record.status), \(dropped) dropped so far). "
                + "The mission record is durable; her recollection of THIS one is not. "
...
final class WorkshopMissionMemoryGate: @unchecked Sendable {
