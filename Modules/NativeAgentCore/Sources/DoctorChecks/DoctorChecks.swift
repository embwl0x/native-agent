...
// MARK: - InstalledAppZeroPythonCheck

public struct InstalledAppZeroPythonCheck: DoctorCheck {
    public let id: String = "installed_app_zero_python"
    public let title: String = "Installed App Zero-Python"
    private let appURL: URL

    public init(appURL: URL = InstalledAppZeroPythonCheck.defaultAppURL()) {
        self.appURL = appURL
    }

    /// Prefer the ACTUALLY-RUNNING app bundle so a normal /Applications install
    /// (public DMG) is checked, not a hardcoded ~/Applications dev path (which a
    /// public user doesn't have → a false "not found" warning). Falls back to
    /// ~/Applications only when the running bundle can't be resolved (e.g. a CLI
    /// harness). 2026-07-04.
    public static func defaultAppURL() -> URL {
    ...
    public func run() async -> CheckResult {
        let executable = appURL
            ...
            .appendingPathComponent("NativeAgentApp")
        guard FileManager.default.fileExists(atPath: executable.path) else {
            return CheckResult(
                id: id,
                title: title,
                status: "warn",
                detail: "Installed app executable was not found at \(executable.path).",
                repair: "Run ./script/install_app.sh from the repo to build and install NativeAgent.app."
            )
        }
        let badNeedles = ["native_agentd.py", "Python.framework", "python3", "site-packages", "sentence-transformers"]
        var matches: [String] = []
        if let enumerator = FileManager.default.enumerator(at: appURL, includingPropertiesForKeys: nil) {
            while let url = enumerator.nextObject() as? URL {
                let path = url.path
                if badNeedles.contains(where: { path.contains($0) }) {
                    matches.append(path.replacingOccurrences(of: appURL.path + "/", with: ""))
        ...
        if !matches.isEmpty {
            return CheckResult(
                ...
                title: title,
                status: "fail",
                detail: "Installed app contains retired Python artifact(s): \(matches.joined(separator: ", ")).",
                repair: "Rebuild/install the Swift app bundle; Doctor will not mutate a signed app bundle in place."
            )
        }
        return CheckResult(
            id: id,
            title: title,
            status: "ok",
            detail: "Installed NativeAgent.app exists and no retired Python artifacts were found."
        )
    }
}

// MARK: - RetiredDaemonProcessCheck

public struct RetiredDaemonProcessCheck: RepairingDoctorCheck {
    public let id: String = "retired_python_daemon"
    public let title: String = "Retired Python Daemon"

    public init() {}

    public func run(repair: Bool) async -> CheckResult {
        let pids = Self.pgrepNativeAgentDaemon()
        if pids.isEmpty {
            return CheckResult(
                id: id,
                title: title,
                status: "ok",
                detail: "No retired native_agentd.py process is running."
            )
        }
        if repair {
            var killed: [String] = []
            for pid in pids {
                // Re-verify the argv immediately before the kill: the pid
                // could have been recycled between detection and now, and
                // SIGTERM to the wrong process is unrecoverable.
                guard Self.argvIsPythonRunningDaemon(Self.commandLine(of: pid) ?? "") else { continue }
                if kill(pid, SIGTERM) == 0 {
                    killed.append(String(pid))
                }
            }
            let after = Self.pgrepNativeAgentDaemon()
            if after.isEmpty {
                return CheckResult(
                    id: id,
                    title: title,
                    status: "ok",
                    detail: "No retired native_agentd.py process is running after repair.",
                    repair: "Sent SIGTERM to retired daemon pid(s): \(killed.joined(separator: ", "))."
                )
            }
            return CheckResult(
                id: id,
                title: title,
                status: "fail",
                detail: "Retired native_agentd.py process still running: \(after.map(String.init).joined(separator: ", ")).",
                repair: killed.isEmpty ? "No retired daemon process accepted SIGTERM." : "Sent SIGTERM to: \(killed.joined(separator: ", "))."
            )
        }
        return CheckResult(
            id: id,
            title: title,
            status: "fail",
            detail: "Retired native_agentd.py process is running: \(pids.map(String.init).joined(separator: ", ")).",
            repair: "Run Repair Safe Issues to stop retired native_agentd.py processes."
        )
    }

    /// Find pids of a python interpreter actually EXECUTING native_agentd.py.
    ///
    /// AUDIT FIX (2026-06-10): the old `pgrep -f native_agentd.py` substring-
    /// matched ANY argv mentioning the file — `vim native_agentd.py`,
    /// `grep native_agentd.py ...`, an audit session — and repair SIGTERMed
    /// every match. Two layers now:
    ///   1. pgrep pattern anchored to a python token preceding the script
    ///      (`python[0-9.]* .*native_agentd\.py`).
    ///   2. per-pid argv verification via `ps -o command=`: the FIRST token
    ///      (the executable) must contain "python" AND a LATER token must be
    ///      the native_agentd.py script path. This kills the remaining
    ///      false-positive class (e.g. `bash -c '... python3 native_agentd.py'`
    ///      wrappers, grep patterns quoting the full command).
    /// AUDIT FIX (2026-07-21): bounded wait for the pgrep/ps probes. Both
    /// probes previously ran a bare `waitUntilExit()` with no timeout and no
    /// escalation — one hung probe wedged the whole sequential doctor run.
    /// TERM at the deadline, SIGKILL after a 1s grace; returns false when the
    /// wait had to be forced (callers treat the probe as failed). Internal
    /// (not private) for direct test coverage, mirroring
    /// `argvIsPythonRunningDaemon`.
    static func boundedWaitUntilExit(_ process: Process, timeout: TimeInterval) -> Bool {
        func pollUntil(_ deadline: Date) -> Bool {
            while process.isRunning && Date() < deadline {
                Thread.sleep(forTimeInterval: 0.02)
            }
            return !process.isRunning
        }
        if pollUntil(Date().addingTimeInterval(timeout)) { return true }
        process.terminate()
        if pollUntil(Date().addingTimeInterval(1.0)) { return false }
        _ = Darwin.kill(process.processIdentifier, SIGKILL)
        _ = pollUntil(Date().addingTimeInterval(1.0))
        return false
    }

    static func pgrepNativeAgentDaemon() -> [pid_t] {
        let process = Process()
        process.executableURL = URL(fileURLWithPath: "/usr/bin/pgrep")
        process.arguments = ["-f", "python[0-9.]* .*native_agentd\\.py"]
        let pipe = Pipe()
        process.standardOutput = pipe
        process.standardError = Pipe()
        do {
            try process.run()
            // Bounded wait (audit fix 2026-07-21) — a hung pgrep must fail
            // the probe, not wedge the doctor run.
            guard boundedWaitUntilExit(process, timeout: 10) else { return [] }
            ...
            return text
                ...
                .filter { $0 != selfPid }
                .filter { argvIsPythonRunningDaemon(commandLine(of: $0) ?? "") }
        } catch {
            return []
        }
    }

    /// Pure argv classifier (internal for direct test coverage): true iff the
    /// first token is a python interpreter and a subsequent token is the
    /// native_agentd.py script.
    static func argvIsPythonRunningDaemon(_ argv: String) -> Bool {
        let tokens = argv.split(separator: " ", omittingEmptySubsequences: true)
        guard let first = tokens.first?.lowercased(), first.contains("python") else {
            return false
        }
        return tokens.dropFirst().contains {
            $0 == "native_agentd.py" || $0.hasSuffix("/native_agentd.py")
        }
    }

    /// Full command line of `pid` via `ps -o command=`, or nil if the pid is
    /// gone / ps fails.
    static func commandLine(of pid: pid_t) -> String? {
        ...
        do {
            try process.run()
            // Bounded wait (audit fix 2026-07-21) — a hung ps must fail the
            // probe, not wedge the doctor run.
            guard boundedWaitUntilExit(process, timeout: 10) else { return nil }
...
public actor SwiftNativeDoctorChecks: DoctorChecksProtocol {
    ...
    public init(checks: [DoctorCheck] = [
        ...
        InstalledAppZeroPythonCheck(),
        RetiredDaemonProcessCheck(),
    ]) {
        self.checks = checks
    }
