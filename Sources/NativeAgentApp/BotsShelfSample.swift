...
enum BotsShelfSample {
    static var records: [BotsShelfRecord] {
        ...
        let names = ["Release notes and compatibility across the desktop and mobile apps, including setup changes and migration notices", "Trail conditions", "Reading list"]
        ...
        return names.indices.map { index in
            ...
            let health: [ShelfRunHealth] = index == 0 ? [.partial, .nothingNew, .nothingNew, .failed, .ok, .nothingNew, .ok] : (index == 1 ? [.failed, .ok] : [.nothingNew])
            let entries = health.enumerated().map { offset, health in
                let at = day.addingTimeInterval(Double(2 - offset) * 86400)
                return ShelfEntry(botId: bot.id, briefVersion: 1, runAt: at,
