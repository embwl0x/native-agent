...
enum BackgroundLoopsAssembly {
    ...
    static func assembleAllLoops(
        ...
        var loops: [any LoopRunner] = [
            ...
            // A5.5: the delete-side sibling of the watchdog above — orphaned
            // daemon-era context receipts and month-old *.bak backups. REPORT-ONLY
            // until `staleArtifactSweepEnabled` is set; every removal leaves a
            // receipt in logs/maintenance_sweep.jsonl.
            makeStaleArtifactSweepLoop(dataRoot: dataRoot),
            ...
            // MEASURE v2 (north-star): weekly golden-eval submission for a
            // CONTROLLED scoreboard cohort. OFF by default — submits
            // token-spending missions only when `goldenEvalEnabled` is set.
            makeGoldenEvalLoop(dataRoot: dataRoot),
